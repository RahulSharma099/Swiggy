...
export function createMetricsRoutes(metrics: MetricsCollector): Router {
  ...
  router.get('/summary', (_req, res: Response) => {
    ...
    for (const [key, value] of Object.entries(allMetrics.counters || {})) {
      ...
      if (key.includes('event_') && key.includes('published')) summary.counters.total_events_published += value;
    ...
    for (const [key, value] of Object.entries(allMetrics.gauges || {})) {
      if (key.includes('websocket')) summary.gauges.active_websocket_connections = value;
      if (key.includes('handler')) summary.gauges.registered_event_handlers = value;
    }
