...
export const createWorkspaceRoutes = (
  ...
  router.post("/", auth.requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const workspace = await services.workspace.createWorkspace(
        { name },
        userId,
      );
  ...
  router.get(
    "/:id",
    auth.requireWorkspaceMember("id"),
  ...
  router.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const workspace = await services.workspace.updateWorkspace(
          id,
          updates,
          userId,
        );
  ...
  router.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const project = await services.project.updateProject(
          id,
          updates,
          userId,
        );
  ...
  router.get("/", auth.requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!projectId) {
        return res
          .status(400)
          .json({ error: "projectId query parameter required" });
      }

      const issues = await services.issue.getProjectIssues(
        projectId as string,
        {
...
export const createSprintRoutes = (
  ...
  router.get(
    ...
    auth.requireAuth,
    sprintHandlers.getActiveSprint,
  );
...
export const createCommentRoutes = (
  ...
  router.get(
    "/user/comments",
    auth.requireAuth,
    commentHandlers.getUserComments,
  );
   ...
   */
  router.get(
    "/:issueId/comments/search",
    auth.requireAuth,
    commentHandlers.searchComments,
  );
  ...
  router.get(
    ...
    auth.requireAuth,
    commentHandlers.getActivityFeed,
  );
 ...
 */
export const createSearchAggregatorRoutes = (
  deps: AppDependencies,
  auth: AuthMiddleware,
) => {
 ...
 */
export const createSearchAnalyticsRoutes = (
  deps: AppDependencies,
  auth: AuthMiddleware,
) => {
